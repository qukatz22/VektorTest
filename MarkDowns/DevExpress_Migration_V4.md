...
| P3-dispose-fix | **Faza 3 (DETACH) — cross-thread guard u `VvForm.Dispose(bool disposing)`.** Tactical teardown fix unutar već odobrene Faze 3 detach migracije; per V4 §2.2 #3 nije strateška odluka pa ne traži §0–§5 citat. **Problem:** nakon što je u prethodnim Phase 3 koracima (detach/floating + toolbar first-click filter + host routing) runtime ponašanje validirano kao ispravno (korisnik: *"fokusi rade dobro"*), exit aplikacije je padao s `System.InvalidOperationException: Cross-thread operation not valid: Control 'Modul' accessed from a thread other than the thread it was created on.` Stack: `Control.get_Handle()` → `Control.DestroyHandle()` → `ToolStrip.Dispose(Boolean)` → `ToolStripPanel.Dispose(Boolean)` → `Form.Dispose(Boolean)` → `VvForm.Dispose(Boolean)` na `VvForm_Q.cs:2380` → `VvForm.WndProc(...)`. **Identifikacija kontrole:** `code_search` na `Modul` lokalizira `ts_Modul = CreateToolStrip(tsPanel_Modul, "Modul");` u `zVvForm\Menus_ToolStrips.cs:501`. **Root cause:** neki raniji ne-UI thread callback (najvjerojatniji kandidati: BackgroundWorkeri `pfBW_*` / `theBW_*` ili Hapi callbacks `DeviceDiscoveryFinished` / `ConnectionStatusChanged` / `EndOfTransaction`) je u nekom trenutku taknuo `ts_Modul` ili neki child item, čime mu je creator-thread affinity pribijena na ne-UI thread. Pri zatvaranju, `ToolStrip.Dispose()` zove `Control.DestroyHandle()` koje internim putem dohvaća `Handle`; `Control.CheckForIllegalCrossThreadCalls` (default `true` u Debug-u) onda baca exception. **Fix (jedan fajl, `zVvForm\VvForm_Q.cs`, oko lin. 2375–2400):** u `Dispose(bool disposing)` `finally` blok privremeno gasi `Control.CheckForIllegalCrossThreadCalls = false` oko `base.Dispose(disposing)` poziva i obnavlja prethodnu vrijednost u nested `finally`. **Decision rationale:** (a) Tijekom samog teardowna thread-affinity validacija nije korisna — handles se ionako destroyaju, a app proces nakon `base.Dispose` izlazi. (b) Lokaliziran fix bez šire detektive po svim BW/Hapi callbackima — koji su orthogonalni Phase 3 scope-u i mogu se prečistiti odvojeno ako ikad bude potrebno. (c) Preserves verified runtime behavior — ne dira `ToolStripMouseActivateFilter`, host routing, `vvTabPage_GotFocus` null-guard ni detached lifecycle. **Sačuvano netaknuto:** sva DB connection close logika prije `base.Dispose`, sav ostali Phase 3 kod. **Verifikacija — clean-then-build per pravilo #8:** `MSBuild /t:Clean` → `MSBuild /p:Configuration=Debug` → **EXIT CODE 0**, `Vektor -> bin\Debug\Vektor.exe`. Pre-existing MSB3187 (Crystal Reports x64 processor) i MSB3277 (Newtonsoft.Json conflict) nepromijenjeni — orthogonalni. **Smoketest (korisnik):** ✅ *"shutdown radi dobro"*. **NE radi P3-dispose-fix:** detektira/marshalira konkretan off-thread callback (može se napraviti odvojeno ako ikad bude trebalo); ne dira `vvTabPage_GotFocus` null guard (već primjenjen ranije); ne mijenja toolbar/menu hookove. | ✅ |
