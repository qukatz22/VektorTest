...
| C12 | **Faza 1c — `VvUserControl` decoupling (R3 mitigacija).** `Framework\VvUserControl.cs`: (1) dodan settable `IVvDocumentHost DocumentHost` s backing fieldom `_documentHost`; getter vraca `_documentHost ?? (IVvDocumentHost)ZXC.TheVvForm` — nulta promjena za postojece call-siteove, dok u Fazi 3 `VvFloatingForm` postavlja sebe kao host. (2) `TheVvTabPage` konvertiran iz read-only u settable property s backing fieldom `_theVvTabPage`; getter prvo provjerava backing field, tek onda pada na postojeci `(VvTabPage)this.Parent.Parent` — u Fazi 3 reparent UC-a u `VvFloatingForm` ne lomi navigaciju jer setter cuva referencu na ishodisni tab. (3) `TheDbConnection` fallback lanac prosiren: `VvFindDialog` / null-tab grana sada ide `DocumentHost?.TheDbConnection ?? ZXC.TheVvForm.TheVvTabPage.TheDbConnection` (prije: direktno u globalni `ZXC.TheVvForm`); normalna grana (`this.TheVvTabPage.TheDbConnection`) nepromijenjena — `TheVvTabPage` getter vec respektira setter, pa reparent automatski rutira na ishodisni tab. **Fallback-safe dizajn:** nijedan od tri property-ja ne mijenja ponasanje dok nije eksplicitno postavljen, pa svi ~40 DUC-ova i business call-siteovi nastavljaju raditi bez ikakve izmjene. Adresa R3 — najopasnija sprega za Fazu 3 (Parent.Parent puca pri reparent-u). Otkljucava Fazu 1d (Rtrans decoupling kroz `DocumentHost`). Build green. | ✅ |
