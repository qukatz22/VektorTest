...
| C9 | **VvToolbarFactory skeleton.** Kreiran `Framework\VvToolbarFactory.cs` — static klasa sa signature-stubovima: `CreateBar_Record`, `CreateBar_Report`, `CreateBar_SubModul`, `CreateMenuBar(host, isDetached=false)`, `ApplyWriteMode(host, ZXC.WriteMode)`, `ApplyProductTypeFilter(host)`. `Create*` metode bacaju `NotImplementedException` (Faza 2g implementacija). `ApplyWriteMode` u C9 **privremeno delegira** na postojeću `VvForm.SetVvMenuEnabledOrDisabled_RegardingWriteMode(writeMode)` metodu kada je host `VvForm` — svrha je omogućiti *novim* call-siteovima (kasnije u 1d / business layer) da već sad idu kroz factory, pa se u C10 samo tijelo mijenja (ne i call-siteovi). Za ne-`VvForm` host (detached `VvFloatingForm` u Fazi 3) baca `NotImplementedException` dok C10 ne migrira tijelo iz `SetVvMenuEnabledOrDisabled_RegardingWriteMode_JOB` (svih 7 specijalnih slučajeva iz §1.6). Postojeći `VvForm.SetVvMenuEnabledOrDisabled_*` call-siteovi unutar VvForm partial fileova **nisu dirani** — i dalje zovu direktno. Build green. | ✅ |
