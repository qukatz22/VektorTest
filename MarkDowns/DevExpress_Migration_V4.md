...
| C15 | **Faza 1e — per-host flag bucket infrastruktura (§1.14, §3.1e).** Formaliziran contract za ~20 `*_InProgress` flagova klasificiranih kao per-DocumentHost (Record-level × 5, Cross-DUC copy × 3, UI state × 5, RISK field ops × 7). **Option B disciplina** (isti princip kao C10 za `ApplyWriteMode`): u C15 SAMO infrastruktura — klasa + interface property + ResetAll path — tijela flagova (read/write na ~40 DUC call-siteovima) **ostaju u ZXC staticima do Faze 3**. Flip call-siteova dogodit ce se u Fazi 3 kad `VvFloatingForm` postane aktivan; do tada infrastruktura je testirana ali dormant. **Promjene:** (1) `Framework\VvPerHostState.cs` **kreiran** — global namespace, 20 bool polja (grupirana po §1.14 kategorijama) + `ResetAll()`. (2) `Framework\IVvDocumentHost.cs` — dodan `VvPerHostState PerHost { get; }` contract. (3) `zVvForm\VvForm_IVvDocumentHost.cs` — `private readonly VvPerHostState _perHost = new VvPerHostState();` + explicit impl `VvPerHostState IVvDocumentHost.PerHost => _perHost;`. (4) `Framework\ZXC.cs` — dodan `ActivePerHostState` convenience getter (vraca `(ActiveDocumentHost as IVvDocumentHost)?.PerHost`, null-safe). (5) `Reset_PerHost_StatusVariables_ForAllHosts()` **rewired** — primarni path sada iterira `DocumentHosts` i zove `host.PerHost.ResetAll()`; defanzivni ZXC static resetovi ostaju ispod kao fallback do Faze 3 flipa. **Svjesno izostavljeno iz C15:** revizija `ShouldSupressRenewCache` (§1.14 / §3.1e task). Logicki hazard: kad bi se prepravilo na `ActivePerHostState?.RISK_CheckPrNabDokCij_inProgress ?? RISK_CheckPrNabDokCij_inProgress`, svaki novi prazan bucket (uvijek `false`) bi null-coalescingom pao natrag na ZXC statik — ali kroz `?.` bi skrio legitimne `true` vrijednosti ako bucket postane primarni prije nego se call-siteovi flipnu. Sigurnije: `ShouldSupressRenewCache` ostaje netaknut u C15, revizija se radi u Fazi 3 kad bodies flipaju \u2014 tad ce imati semantiku `ActivePerHostState.RISK_CheckPrNabDokCij_inProgress \|\| ... \|\| RISK_DisableCacheTemporarily` (bez fallback-a, jer buckets postaju authoritative). **Zero behavioral change:** nijedan call-site nije diran; build green. | \u2705 |
