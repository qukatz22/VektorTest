...
| C19 | **Faza 2b — `VvForm` base class swap: `Crownwood.DotNetMagic.Forms.DotNetMagicForm` → `DevExpress.XtraEditors.XtraForm`.** Prva strukturna promjena UI hijerarhije u Fazi 2; glavna forma sada je DX-form. **Census prije izmjena (autoritativno enumerirano):** 10 `partial class VvForm` deklaracija u workspaceu — 9 ih je nosilo `: DotNetMagicForm` base (s različitim interface-suffixima); 1 (`zVvForm\VvForm_IVvDocumentHost.cs`) ima samo `: IVvDocumentHost` bez base i ostavljen je netaknut. **Promjene:** (1) **9 base swap-ova** — `zVvForm\VvForm.cs` (s `IVvDocumentHost` suffix), `Initializations_Settings.cs`, `Menus_ToolStrips.cs`, `Moduls_CommandPanel.cs`, `OnClick_EventHandlers.cs`, `SubModulActions.cs`, `TabControl_TabPages.cs`, `VvForm_Q.cs` (s `Events.Required, Events.Status` suffix), `UtilsEtc\F2eRacuni.cs` (FQN-style declaration). U svim slučajevima korišten **fully-qualified** `DevExpress.XtraEditors.XtraForm` umjesto dodavanja novog `using DevExpress.XtraEditors;` per fajl — najmanji diff, dosljedno F2eRacuni FQN stilu, ne dira postojeće `using Crownwood.DotNetMagic.Forms;` direktive (koje ostaju do Faze 2c-2g jer drugi tipovi iz tog namespacea — TabControl, TabPage, TitleBar, ButtonWithStyle — još su u upotrebi). (2) **5 `Form.Style` decommissioning lokacija** — XtraForm ne nudi Crownwood-specific `Style` property tipa `VisualStyle`. Komentirana 4 `this.Style = ZXC.vvColors.vvform_VisualStyle;` write-a (Initializations_Settings.cs:447, OnClick_EventHandlers.cs:794 i 865) i 1 `ZXC.TheVvForm.Style = ...` u `UtilsEtc\VvColorsStylsDlg.cs:152`. Compound assignment `ZXC.vvColors.vvform_VisualStyle = this.Style = vvEnvDescriptor.VvColorsAndStylesEnv.vvform_VisualStyle;` u Initializations_Settings.cs:863 razdvojen — middle `this.Style =` dropped, lijevi i desni operand zadržani. **Sačuvano (svjesno):** `ZXC.vvColors.vvform_VisualStyle` field i sve `tabControlColors.Style` reference — Crownwood child controls (`TheTabControl`, `TreeView_Modul`) i dalje su Crownwood do Faza 2c/2h, pa im treba VisualStyle infrastruktura. Faza 2i (VvColors → DX SkinStyle) preuzima cijelu konverziju. **Tool discipline (kritično — pouka iz prvog failed pokušaja):** isključivo `multi_replace_string_in_file` (14 izmjena u 1 batch-u, 14/14 OK); zero PowerShell `WriteAllText` (prošli pokušaj koristio je `WriteAllText` s UTF-8 encoding-om što je destruktivno overwrite-alo 5 UTF-16 LE fajlova prepolovivši im byte count — abandoned i revertan). **Encoding verifikacija:** BOM signature inspection nakon izmjena potvrdila — `Initializations_Settings.cs`, `Menus_ToolStrips.cs`, `Moduls_CommandPanel.cs`, `OnClick_EventHandlers.cs` ostali UTF-16 LE (`FF FE` BOM intact); `VvForm.cs`, `SubModulActions.cs`, `TabControl_TabPages.cs`, `VvForm_Q.cs`, `VvColorsStylsDlg.cs` ostali UTF-8 noBOM; `F2eRacuni.cs` ostao UTF-8 BOM. Byte rast u UTF-16 fajlovima paran (npr. +588, +32, +32, +292) = potvrda da je 2-byte-per-char encoding očuvan. `git diff --stat` prikazuje "Bin" za 4 UTF-16 fajla, ali to je **kozmetička posljedica gitovog default content sniffera** (BOM `FF FE` ga zbunjuje), **nije oštećenje**. **Verifikacija — clean-then-build per pravilo #8:** `MSBuild /t:Clean` → `MSBuild /p:Configuration=Debug` → **EXIT CODE 0**, zero CS errors, `Vektor -> bin\Debug\Vektor.exe`. Pre-existing warnings nepromijenjeni. **Smoketest (korisnik, F5):** ✅ aplikacija startira, glavna forma se prikazuje s **Office 2019 Colorful** skinom (DX skin engine sad ima kontrolu nad form chrome jer XtraForm poštuje `UserLookAndFeel.Default` postavljen u C18), tabovi otvaraju, login radi, F2 modul radi. Crownwood `TabControl` i dalje hosta unutarnje module (host-child kompatibilnost potvrđena: XtraForm hosta bilo koji `Control`-derived child). **NE radi C19:** TabControl swap (Faza 2c), TabPage swap (Faza 2d), MenuStrip/ToolStrip → BarManager (Faza 2g), TreeControl swap (Faza 2h), VvColors konverzija (Faza 2i), Crownwood DLL uklanjanje (Faza 2k). | ✅ |
