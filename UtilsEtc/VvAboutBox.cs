      ...
      if(ApplicationDeployment.IsNetworkDeployed)
      {
         ...
         if(info.UpdateAvailable)
         {
            ...
            if(doUpdate)
            {
               ...
               {
                  ad.Update();
                  ZXC.aim_emsg("Aplikacija je obnovljena, te æe biti restartana.");
                  Application.Restart();
               }
               catch(DeploymentDownloadException dde)
               {
...
public class VvMessageBox_UC : UserControl
{
   ...
   private void CreateHamper_SELECT(out VvHamper hamper)
   {
      ...
      btn_Select_All       = hamper.CreateVvButton(0, 0, new EventHandler(Select_All      ), "Oznaèi sve");
      btn_Deselect_All     = hamper.CreateVvButton(1, 0, new EventHandler(Deselect_All    ), "Odznaèi"   );
   ...
   private void CreateHamper_F2_SEND_candidates(out VvHamper hamper)
   {
      ...
      tbx_numOfFirstLinesOnly_SEND.JAM_CharEdits = ZXC.JAM_CharEdits.DigitsOnly;
                                     hamper.CreateVvLabel  (2, 0, " raèuna", ContentAlignment.MiddleLeft);
   ...
   private void CreateHamper_F2_SEND_stop(out VvHamper hamper)
   {
    ...
    //hamper.VvRowHgt       = new int[] { ZXC.QUN-ZXC.Qun4, ZXC.QUN - ZXC.Qun4};
      ...
      cbx_IsAutoSend = hamper.CreateVvCheckBox_OLD(0, 0, null, "Ubuduæe, Automatski pošalji eRaèune"           , RightToLeft.No);
   ...
   private void CreateHamper_F2_MAP_stop(out VvHamper hamper)
   {
      ...
      cbx_isAutoMAP = hamper.CreateVvCheckBox_OLD(0, 0, null, "Ubuduæe, Automatski prijavi uplate (prilikom otvaranja FIR-a, a uz dodatnu potvrdu)", RightToLeft.No);
   ...
   private void CreateHamper_F2_IMPORT_stop(out VvHamper hamper)
   {
    ...
    //hamper.VvRowHgt       = new int[] { ZXC.QUN-ZXC.Qun4, ZXC.QUN - ZXC.Qun4};
      ...
      cbx_isAutoImport = hamper.CreateVvCheckBox_OLD(0, 0, null, "Ubuduæe, Automatski preuzmi ulazne eRaèune"      , RightToLeft.No);
   ...
   private void CreateMultiColumn_F2_SEND_candidates(VvDataGridView theGrid)
   {
      ...
      CreateColumn_skip    (theGrid, "Preskoèi"    , ZXC.Q3un        );
   ...
   private void CreateMultiColumn_F2_IMPORT_candidates(VvDataGridView theGrid)
   {
      ...
      CreateColumn_datumUpl(theGrid, "Datum raèuna ", ZXC.Q4un + ZXC.Qun2);
      CreateColumn_partner (theGrid, "Dobavljaè"    , ZXC.Q10un + ZXC.Q3un);
...
//    vvtb_kol = theGrid.CreateVvTextBoxFor_String_ColumnTemplate("vvtb_kol", null, -12, header);
   ...
   //{
   //  if(rowIdx == 0)TheGrid.PutCell(ci.iT_message , rowIdx, "Servis e-Raèun za državu je internetski servis koji omoguæuje razmjenu strukturiranih elektronièkih raèuna.Servis je u potpunosti usklaðen sa zakonskom regulativom.Servis omoguæuje upravljanje poslovnim procesom izdavanja, zaprimanja, prihvaæanja ili odbijanja zaprimljenog raèuna te arhiviranja istog na brži i sigurniji naèin." );
   //  if(rowIdx == 1)TheGrid.PutCell(ci.iT_message , rowIdx, "Korisnici web aplikacije trebaju imati digitalne certifikate na kripto ureðaju za zaprimanje raèuna te njihov prihvat ili odbijanje.Za postupak likvidature korisnici mogu koristiti digitalne certifikate na kripto ureðaju ili poslovne soft certifikate.Da bi korisnik mogao zaprimiti raèun putem web aplikacije, barem jedna osoba treba imati certifikate na kripto ureðaju kojoj æe se dodijeliti pravo za Pregled i prihvat raèuna." );
   //  if(rowIdx == 2)TheGrid.PutCell(ci.iT_message , rowIdx, "Što je web aplikacija e-Raèuna za državu?");
   //  if(rowIdx == 3)TheGrid.PutCell(ci.iT_message , rowIdx, "Niste obvezni koristiti likvidaturu putem web aplikacije e-Raèun za državu. Servis e-Raèun za državu je internetski servis koji omoguæuje razmjenu strukturiranih elektronièkih raèuna.Servis je u potpunosti usklaðen sa zakonskom regulativom.Servis omoguæuje upravljanje poslovnim procesom izdavanja, zaprimanja, prihvaæanja ili odbijanja zaprimljenog raèuna te arhiviranja istog na brži i sigurniji naèin.");
