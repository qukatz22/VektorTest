...
"HAVING ABS (r1.t_cij - TheCIJ) > " + RtransDao.ChkPrNbC_diff_tolerancy + "                                                                                                                                     " + "\n";
   ...
   internal static XSqlCommand Get_Zatvaranja_FtransList_byDate_Command(XSqlConnection conn, DateTime dateOD, DateTime dateDO)
   {
      XSqlCommand cmd = InitCommand(conn);
      ...
      cmd.CommandText =
            ...
            "AND ftr.t_dokDate >= ?prm_theDateOd AND ftr.t_dokDate <= ?prm_theDateDo " + "\n" +

            "AND ftr.t_tt IN" + TtInfo.GetSql_IN_Clause(Ftrans.WantedMAP_TTsStringArray); // + " \n" +

      //"AND xtr.recID IS NULL   " + "\n\n" ;

      CreateCommandParameter(cmd, "theDateOd", dateOD, XSqlDbType.Date, 32);
      CreateCommandParameter(cmd, "theDateDo", dateDO, XSqlDbType.Date, 32);
