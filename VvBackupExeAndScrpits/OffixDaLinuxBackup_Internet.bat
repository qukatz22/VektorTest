...
set projName=INTRADE

set maxmailsize=10000000

SET backupdir=F:\Offix BACKUP\2010
...
::====================================================================== 
FOR /f %%i IN ('doff.exe %theDatePattern%') DO SET theYYYYMMDD=%%i

SET todayFiles="%backupdir%\*%theYYYYMMDD%*.Z"
ECHO.
::====================================================================== 
IF EXIST %todayFiles% (
...
IF /i %~z1 LSS %maxmailsize% (
   ...
   blat -q -attach %1 -subject "BkpFile: [%recent%]" -body " " -server %outgoingMailServer% -f %fromEmail% -to %toEmailWattachment%
) ELSE (
